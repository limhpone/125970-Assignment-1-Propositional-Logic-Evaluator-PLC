...
The implementation applies compiler front-end techniques~\citep{aho2006compilers}: lexical analysis, parsing with syntax-directed AST construction, and AST-based evaluation and translation. The GUI is built with PySide6 (Qt6 for Python) and supports both Unicode ($\wedge$, $\vee$) and ASCII (\texttt{\&}, \texttt{|}) operator input. Negation ($\neg$), implication ($\rightarrow$), and biconditional ($\leftrightarrow$) are outside the scope of this assignment.
...
The assignment requires expressions built from \texttt{t}, \texttt{f}, $\wedge$, and $\vee$ with $\wedge$ binding tighter than $\vee$. In addition, this implementation supports parenthesised grouping as an extension: if $B$ is an expression then $(B)$ is an expression, allowing the user to explicitly override default precedence.
...
\textbf{Conjunction} ($\wedge$) yields \texttt{t} only when both operands are \texttt{t}; \textbf{disjunction} ($\vee$) yields \texttt{t} when at least one operand is \texttt{t}.
...
\textbf{Ambiguity.} Rules (2) and (3) make the grammar ambiguous because the expression \texttt{t $\vee$ f $\wedge$ f} admits two distinct parse trees: one where $\vee$ is the root (grouping as $\texttt{t} \vee (\texttt{f} \wedge \texttt{f})$) and one where $\wedge$ is the root (grouping as $(\texttt{t} \vee \texttt{f}) \wedge \texttt{f}$).

\subsection{Equivalent Unambiguous Grammar}

The standard compiler-theory technique for eliminating this ambiguity is \emph{grammar stratification\/}~\citep{aho2006compilers}: a separate non-terminal is introduced for each precedence level so that the grammar structure itself enforces the correct grouping. The equivalent unambiguous grammar is:

\begin{align}
E &\rightarrow E \;\vee\; T \;\mid\; T \label{eq:strat_e}\\
T &\rightarrow T \;\wedge\; F \;\mid\; F \label{eq:strat_t}\\
F &\rightarrow \texttt{(}\; E \;\texttt{)} \;\mid\; \texttt{t} \;\mid\; \texttt{f} \label{eq:strat_f}
\end{align}

Here $E$ handles disjunction (lowest precedence), $T$ handles conjunction (higher precedence), and $F$ represents atomic values and parenthesised sub-expressions. Left recursion in (\ref{eq:strat_e}) and (\ref{eq:strat_t}) encodes left-associativity. Because each operator appears at exactly one level, the grammar admits a unique parse tree for every valid input.

In this implementation, the ambiguous grammar from Table~\ref{table:grammar_rules} is retained and the ambiguity is instead resolved by declaring operator \textbf{precedence} and \textbf{associativity} in the LALR(1) parser (Section~\ref{sec:precedence}). SLY's precedence mechanism achieves the same disambiguation automatically, while keeping the production rules simpler and closer to the original language definition~\citep{beazley2023sly}.
...
Prefix notation eliminates the need for precedence rules because the grouping structure is fully explicit through parenthesisation.
