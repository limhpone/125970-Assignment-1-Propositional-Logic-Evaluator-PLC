\chapter{LANGUAGE DESIGN AND THEORETICAL FOUNDATIONS}

\section{Propositional Logic}

Propositional logic is a branch of mathematical logic concerned with propositions and their truth values~\citep{ben2012mathematical}. This evaluator supports two binary connectives whose semantics are defined by the truth tables in Table~\ref{table:truth_tables}.
...
Additionally, parenthesised grouping is supported: if $B$ is an expression then $(B)$ is an expression, allowing the user to override default precedence. The operator $\wedge$ has \textbf{higher priority} than $\vee$; for example, the result of \texttt{t $\vee$ f $\wedge$ f} is \texttt{t}.
